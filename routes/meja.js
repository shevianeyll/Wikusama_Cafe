...
app.get("/getAllData", auth, checkRole('admin'), async (req, res) => { // endpoint untuk mengambil semua data meja
  await meja
...
// get data by id meja
app.get("/getById/:id", auth, checkRole('admin'), async (req, res) => { // endpoint untuk mengambil data meja berdasarkan id meja
  await meja
...
// get meja by status
app.get("/getByStatus/:status", auth, checkRole('admin'), async (req, res) => { // endpoint untuk mengambil data meja berdasarkan status meja
...
// create meja
app.post("/create", auth, checkRole('admin'), async (req, res) => { // endpoint untuk menambahkan data meja
...
// delete meja
app.delete("/delete/:id_meja", auth, checkRole('admin'), async (req, res) => { // endpoint untuk menghapus data meja
...
// edit meja
app.patch("/edit/:id_meja", auth, checkRole('admin'), async (req, res) => { // endpoint untuk mengubah data meja
  const param = { id_meja: req.params.id_meja }; // inisialisasi parameter yang akan dikirimkan melalui parameter
  ...
  meja.findOne({ where: param }).then((result) => { // mengambil data meja berdasarkan id meja yang dikirimkan melalui parameter
    if (result) { // jika data ditemukan
      if (data.nomor_meja != null) { // jika data nomor meja tidak kosong
        meja
          .findOne({ where: { nomor_meja: data.nomor_meja, status: data.status } }) // mengambil data meja berdasarkan nomor meja yang dikirimkan melalui body request
          .then((result) => { // jika berhasil
            if (result) { // jika nomor meja sudah ada
              res.status(400).json({ // mengembalikan response dengan status code 400 dan pesan nomor meja sudah ada
                status: "error",
...
// search meja
app.get("/search/:nomor_meja", auth, checkRole('admin'), async (req, res) => { // endpoint untuk mencari data meja berdasarkan nomor meja
  meja
